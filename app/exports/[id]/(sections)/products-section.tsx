...
export default function ProductsSection({ exportItem }: ProductsSectionProps) {
  if (!exportItem.products || exportItem.products.length === 0) {
    return null;
  }
    ...
    <div className="w-full">
      <div className="mb-6 sm:mb-8 md:mb-10">
        ...
        <p className="text-[#6C757D] dark:text-white/70 text-base sm:text-lg leading-relaxed">
          Explore our range of {exportItem.title.toLowerCase()} products for {exportItem.destinationRegion}
        </p>
      </div>
      <ProductGrid products={exportItem.products} exportId={exportItem.id} />
    </div>
  );
}

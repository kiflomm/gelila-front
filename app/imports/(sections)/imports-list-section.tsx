...
interface ImportsListSectionProps {
  imports: ImportItem[];
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
      {products.map((product) => (
        ...
        >
          ...
          <Link
            href={`/imports/${importId}/products/${product.id}`}
...
export default function ImportsListSection({ imports, activeRegion }: ImportsListSectionProps) {
  if (!imports || imports.length === 0) {
    return null;
  }

  // Group imports by source region
  const groupedByRegion = imports.reduce((acc, importItem) => {
    const region = importItem.sourceRegion || "Other";
    if (!acc[region]) {
      acc[region] = [];
    }
    acc[region].push(importItem);
    return acc;
    ...
    <div className="mt-4 sm:mt-6 md:mt-8 px-2 sm:px-4 md:px-6 lg:px-8">
      {displayRegions.map((region, regionIndex) => (
        ...
        >
          ...
          {groupedByRegion[region].map((importItem, itemIndex) => (
            <div
              key={importItem.id}

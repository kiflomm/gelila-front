...
const categoryMap: Record<string, string> = newsData.categoryMap || {};
...
export default function NewsGridSection({
  ...
  const { data: newsItems = [], isLoading } = useNewsByCategory(categoryName);

  if (isLoading) {
    return (
      <div className="text-center py-12">
        <p className="text-zinc-600 dark:text-zinc-400">Loading news...</p>
      </div>
    );
  }
